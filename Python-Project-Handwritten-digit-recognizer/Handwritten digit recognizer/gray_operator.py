# This file preprocesses operators data: loads training and testing data for operators, converts from image files to arrays, shuffles the datasets, and saves the datasets using pickle
...
DATADIR = r"MNIST_Dataset_JPG_format\Operators_JPG_training"
DATADIR_TEST = r"MNIST_Dataset_JPG_format\Operators_JPG_testing"
CATEGORIES = ["Plus", "Minus", "Times", "Divide"]   # names of photo files
...
for features, label in training_data:
  X_train_operator.append(features)
  y_train_operator.append(label)

for features, label in testing_data:
  X_test_operator.append(features)
  y_test_operator.append(label)

# Convert lists into arrays for model
X_train_operator = np.array(X_train_operator)
y_train_operator = np.array(y_train_operator)
X_test_operator = np.array(X_test_operator)
y_test_operator = np.array(y_test_operator)
...
pickle_out = open("X_train_operator.pickle", "wb")   
pickle.dump(X_train_operator, pickle_out)
pickle_out.close()

pickle_out = open("y_train_operator.pickle", "wb")
pickle.dump(y_train_operator, pickle_out)
pickle_out.close()

pickle_in = open("X_train_operator.pickle", "rb")
X_train_operator = pickle.load(pickle_in)

# Testing Data Saved and Loaded
pickle_out = open("X_test_operator.pickle", "wb")
pickle.dump(X_test_operator, pickle_out)
pickle_out.close()

pickle_out = open("y_test_operator.pickle", "wb")
pickle.dump(y_test_operator, pickle_out)
pickle_out.close()

pickle_in = open("X_test_operator.pickle", "rb")
X_test_operator = pickle.load(pickle_in)
